...
const ArtistsPage = () => {
  ...
  const filteredArtistsForDeletion = deleteArtistSearch
    ? artists.filter((artist) =>
        artist.name.toLowerCase().includes(deleteArtistSearch.toLowerCase())
      )
    : artists;
  ...
  const handleDeleteArtist = async () => {
    ...
    try {
      ...
      setFilteredArtists((prevFilteredArtists) =>
        prevFilteredArtists.filter((artist) => artist._id !== selectedArtistId)
      );
  ...
  return (
    <div>
      <div className="search-bar-and-buttons">
        <h1 className="title-artists">Artists</h1>
        <p className="p-artists">Here you can find various Artists</p>
        <input
          type="text"
          value={searchInput}
        ...
        >
          ...
          <div>
            <input
              ...
              className="input-in-delete"
            />
          </div>
          <div className="artists-container">
            {filteredArtistsForDeletion.map((artist) => (
              <div key={artist._id} className="artists-delete-grid">
                <p className="artist-container-p">{artist.name}</p>
                <button
                  onClick={() => setSelectedArtistId(artist._id)}
                  className="delete-button-artist"
                >
                  Select for Deletion
                </button>
              </div>
